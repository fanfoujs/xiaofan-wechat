    ...
    message(event) {
      this.setData(
        ...
        () => {
          setTimeout(() => {
            fm.navigateTo(
              `../message/message?id=${event.currentTarget.dataset.user.id}&name=${event.currentTarget.dataset.user.name}`,
            )
          }, 200)
        },
      )
        ...
        wx.showActionSheet({
          ...
          success(result) {
            if (result.tapIndex === 0) {
              fm.post(
                page,
                {status: `@${name} 需要你的微信号，来帮你配置小饭体验者权限。`},
                null,
                () => {
                  wx.showModal({
                    confirmColor: '#33a5ff',
              ...
              fm.post(
                page,
                {
                  status: `@${name} 复制地址并在微信中访问体验小饭：${appidlink}`,
                },
                null,
                () => {
                  wx.showModal({
